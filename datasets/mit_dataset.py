...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 20 21:08:57 2018
...
class MITSceneParsingDataset(torchdata.Dataset):
    """
    addparse : ignore_label=255
    """
    def __init__(self, txt, opt, max_iters= None, max_sample=-1, is_train=1, epoches = 100):
        ...
        #if  max_iters is not None and max_iters > num_sample:
        if self.is_train:
            if  max_iters is not None :
                self.list_sample = self.list_sample * int(self.epoches)
        """
         if  self.is_train and max_iters > 0 :
                self.list_sample = self.list_sample * int(np.ceil(float(max_iters) / num_sample))
        """          
        assert num_sample > 0
    ...
    def _scale_and_corp(self, img, seg, cropSize, is_train):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if self.imgSize > 0:
                img, seg = self._scale_and_corp(img, seg, 
